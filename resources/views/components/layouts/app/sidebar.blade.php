...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    <flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group :heading="__('عمومی')" class="grid">
                <flux:navlist.item icon="home" :href="route('dashboard')" :current="request()->routeIs('dashboard')"
                    wire:navigate>{{ __('Dashboard') }}</flux:navlist.item>
            </flux:navlist.group>
            <flux:navlist.group :heading="__('مدیریت محصولات')" class="grid">
                <flux:navlist.item icon="home" :href="route('products.index')"
                    :current="request()->routeIs('products.index')" wire:navigate>{{ __('محصول ها') }}
                </flux:navlist.item>
            </flux:navlist.group>
        </flux:navlist>
        ...
        <flux:navlist variant="outline">
            <flux:navlist.item icon="user-round-cog" href="{{ route('filament.management.pages.dashboard') }}"
                target="_blank">
                {{ __('پنل مدیریت') }}
            </flux:navlist.item>
        </flux:navlist>
                    ...
                    <div class="p-0 text-sm font-normal">
                        <div class="flex items-center gap-2 px-1 py-1.5 text-start text-sm">
                            ...
                            <div class="grid flex-1 text-start text-sm leading-tight">
                                ...
                                <span class="truncate text-xs">{{ auth()->user()->personal_id }}</span>
                            </div>
                        </div>
                    </div>
                </flux:menu.radio.group>
